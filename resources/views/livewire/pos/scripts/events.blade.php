<script>
	...
	//})
        ...
        window.livewire.on('print-ticket', ticket => {
            var ruta = "{{ url('print/sale')}}" + '/' + ticket
            var w = window.open(ruta, "_blank", "width=100, height=100")
            w.close()

        })

        window.livewire.on('print-last-id', ticketP => {
            console.log('aquia', ticketP)
        var ruta = "{{ url('print/last-sale')}}" + '/' + ticketP
        var w = window.open(ruta, "_blank", "width=100, height=100")
        //w.close()
